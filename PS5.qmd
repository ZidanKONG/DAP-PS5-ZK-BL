...
def enforcement_scraper(month, year):
    ...
    # Start with the first page (page=1)
    page_number = 1
    url = f"{base_url}?page={page_number}"

    while url:  # Loop through pages until no more pages are found
        print(f"Scraping page {page_number}...")  # Progress feedback
        response = requests.get(url)
        soup = BeautifulSoup(response.text, 'lxml')

        # Loop through all enforcement actions on the current page
        for action in soup.find_all('li', class_='usa-card card--list pep-card--minimal mobile:grid-col-12'):
            title_tag = action.find('h2', class_='usa-card__heading')
            title = title_tag.text.strip() if title_tag else 'N/A'
            titles.append(title)
            ...
            try:
                action_date = datetime.strptime(date, '%B %d, %Y')
                if action_date < target_date:
                    continue  # Skip actions older than the target date
            except ValueError:
                continue
            dates.append(date)
        ...
        # Extract agency details for each action
        for link in links:
            action_response = requests.get(link)
            action_soup = BeautifulSoup(action_response.text, 'lxml')
            agency_tag = action_soup.find('span', text='Agency:')
        ...
        # Save data incrementally to CSV after each page
        data = pd.DataFrame({
            'Title': titles,
            'Date': dates,
            'Category': categories,
        ...
        data.to_csv(f'enforcement_actions_{year}_{month}_page_{page_number}.csv', index=False)

        # Reset lists for the next page
        titles.clear()
        dates.clear()
        categories.clear()
        links.clear()
        ...
        # Extract the pagination section to find the next page
        pagination = soup.find('ul', class_='pagination')
        if pagination:
            page_links = pagination.find_all('a', class_='pagination_link')
            print(f"Pagination links: {[link.text.strip() for link in page_links]}")  # Debugging line

            # Find the link to the next page
            next_page_link = None
            for page_link in page_links:
                if page_link.text.strip() == str(page_number + 1):
                    next_page_link = page_link
                    break
            
            # Check if we found the next page link
            if next_page_link:
                page_number += 1
                url = f"{base_url}{next_page_link['href']}"  # Construct the full URL for the next page
                print(f"Next page URL: {url}")  # Debugging line
            else:
                url = None  # No next page found, exit the loop
        else:
            url = None  # Exit if no pagination container is found

    # Return the total number of pages scraped
    return f"Scraped data from {page_number} pages."
```
